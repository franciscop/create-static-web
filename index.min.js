...
function assign$1(object) {
  object = isObject$1(object) ? object : {};
...
var literal = new RegExp(`(?:${quoted.source}|${bool$1.source}|${number.source})`);
...
var boolLine = new RegExp(`^${bool$1.source}$`, 'i');
...
function mkContext(input, line) {
  ...
  var context = underscore.range(begin, end + 1).map(l => [l === line ? '>> ' : '   ', align(l, end), '| ', lines[l - 1]].join('')).join('\n');
...
const AssertionError$1 = error.AssertionError;
...
var Scope = {
  getAll: function () {
    ...
    for (var i = this.scopes.length - 1; i >= 0; i--) {
      underscore.assign(ctx, this.scopes[i]);
    }
    return ctx;
  },
  get: function (str) {
  ...
  push: function (ctx) {
    assert_1(ctx, `trying to push ${ctx} into scopes`);
  ...
  getPropertyByPath: function (scopes, path$$1) {
      ...
      switch (str[i]) {
        case '[':
          ...
          if (/['"]/.test(delemiter)) {
            ...
            assert_1(j !== -1, `unbalanced ${delemiter}: ${str}`);
...
function getValueFromParent(key, value) {
  return key === 'size' && (underscore.isArray(value) || underscore.isString(value)) ? value.length : value[key];
}
...
var factory = function (ctx, opts) {
  ...
  scope.opts = underscore.assign(defaultOptions, opts);
  scope.scopes = [ctx || {}];
  return scope;
};
...
function whiteSpaceCtrl(tokens, options) {
  options = underscore.assign({ greedy: true }, options);
  var inRaw = false;
...
var whitespaceCtrl = whiteSpaceCtrl;

const TokenizationError$1 = error.TokenizationError;




function parse$3(input, file, options) {
  assert_1(underscore.isString(input), 'illegal input');
  ...
  function parseTagToken(raw, value, pos) {
    ...
    if (!match) {
      throw new TokenizationError$1(`illegal tag syntax`, token);
    }
    token.name = match[1];
    token.args = match[2];
    return token;
  ...
  function parseHTMLToken(begin, end) {
    ...
    currIndent = underscore.last(htmlFragment.split('\n')).length;
...
var parse_1 = parse$3;
var whiteSpaceCtrl_1 = whitespaceCtrl;

var tokenizer = {
	parse: parse_1,
	whiteSpaceCtrl: whiteSpaceCtrl_1
};
...
function statFileAsync(path$$1) {
  return new Promise(function (resolve, reject) {
    fs.stat(path$$1, (err, stat) => err ? reject(err) : resolve(stat));
  });
}
var fs_1 = {
  readFileAsync,
...
var operators$1 = function (isTruthy) {
  return {
    '==': (l, r) => l === r,
    '!=': (l, r) => l !== r,
...
const operators$2 = operators$1(isTruthy);



function evalExp(exp, scope) {
  assert_1(scope, 'unable to evalExp: scope undefined');
  ...
  for (var i = 0; i < operatorREs.length; i++) {
    ...
    if (match = exp.match(expRE)) {
      ...
      var op = operators$2[match[2].trim()];
 ...
 */
var loader$1 = function (root, loadImplementation) {
...
var register = loader$1(commonjsGlobal, loadImplementation);
...
function anySeries(iterable, iteratee) {
  var ret = anyPromise.reject(new Error('init'));
...
function mapSeries(iterable, iteratee) {
  var ret = anyPromise.resolve('init');
  var result = [];
  iterable.forEach(function (item, idx) {
    ret = ret.then(() => iteratee(item, idx, iterable)).then(x => result.push(x));
  });
  return ret.then(() => result);
}

var anySeries_1 = anySeries;
var mapSeries_1 = mapSeries;

var promise = {
	anySeries: anySeries_1,
	mapSeries: mapSeries_1
};

const mapSeries$1 = promise.mapSeries;
const RenderBreakError$1 = error.RenderBreakError;
const RenderError$1 = error.RenderError;


var render = {

  renderTemplates: function (templates, scope) {
    assert_1(scope, 'unable to evalTemplates: scope undefined');

    var html = '';
    return mapSeries$1(templates, tpl => {
      return renderTemplate.call(this, tpl).then(partial => html += partial).catch(e => {
        if (e instanceof RenderBreakError$1) {
          e.resolvedHTML = html;
          throw e;
        }
        throw new RenderError$1(e, tpl);
      });
    }).then(() => html);

    function renderTemplate(template) {
      if (template.type === 'tag') {
        return this.renderTag(template, scope).then(partial => partial === undefined ? '' : partial);
      } else if (template.type === 'value') {
        return anyPromise.resolve().then(() => this.evalValue(template, scope)).then(partial => partial === undefined ? '' : stringify(partial));
      } else {
        // template.type === 'html'
        return anyPromise.resolve(template.value);
      }
    }
  },
  ...
  renderTag: function (template, scope) {
    if (template.name === 'continue') {
      return anyPromise.reject(new RenderBreakError$1('continue'));
    }
    if (template.name === 'break') {
      return anyPromise.reject(new RenderBreakError$1('break'));
    }
    return template.render(scope);
  },

  evalValue: function (template, scope) {
    assert_1(scope, 'unable to evalValue: scope undefined');
    return template.filters.reduce((prev, filter) => filter.render(prev, scope), syntax.evalExp(template.initial, scope));
  }
};
...
var tag = function () {
  ...
  var _tagInstance = {
    render: function (scope) {
      var obj = hash$1(this.token.args, scope);
      var impl = this.tagImpl;
      if (typeof impl.render !== 'function') {
        return anyPromise.resolve('');
      }
      return anyPromise.resolve().then(() => impl.render(scope, obj));
    },
    parse: function (token, tokens) {
      ...
      assert_1(tagImpl, `tag ${this.name} not found`);
...
var filter$1 = function (options) {
  options = underscore.assign({}, options);
  var filters = {};
  ...
  var _filterInstance = {
    ...
    parse: function (str) {
      ...
      assert_1(match, 'illegal filter: ' + str);
...
const ParseError$1 = error.ParseError;
...
var parser = function (Tag, Filter) {
  ...
  function parseToken(token, tokens) {
    try {
      ...
      throw new ParseError$1(e, token);
    }
  }
  ...
  function parseValue(str) {
    ...
    assert_1(match, `illegal value string: ${str}`);
...
const lexical$1 = liquidjs.lexical;

const re = new RegExp(`(${lexical$1.identifier.source})\\s*=(.*)`);


var assign$2 = function (liquid) {
  liquid.registerTag('assign', {
    parse: function (token) {
      var match = token.args.match(re);
      assert_1(match, `illegal token ${token.raw}`);
      this.key = match[1];
      this.value = match[2];
    },
    render: function (scope) {
...
const lexical$2 = liquidjs.lexical;
const re$1 = new RegExp(`(${lexical$2.identifier.source})`);
...
var capture = function (liquid) {
  liquid.registerTag('capture', {
    parse: function (tagToken, remainTokens) {
      ...
      assert_1(match, `${tagToken.args} not valid identifier`);

      this.variable = match[1];
      this.templates = [];

      var stream = liquid.parser.parseStream(remainTokens);
      stream.on('tag:endcapture', token => stream.stop()).on('template', tpl => this.templates.push(tpl)).on('end', x => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
      stream.start();
    },
    render: function (scope, hash) {
      return liquid.renderer.renderTemplates(this.templates, scope).then(html => {
...
var _case = function (liquid) {
  liquid.registerTag('case', {
    ...
    parse: function (tagToken, remainTokens) {
      ...
      var p = [];
      var stream = liquid.parser.parseStream(remainTokens).on('tag:when', token => {
        this.cases.push({
          val: token.args,
          templates: p = []
        });
      }).on('tag:else', () => p = this.elseTemplates).on('tag:endcase', token => stream.stop()).on('template', tpl => p.push(tpl)).on('end', x => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
    ...
    render: function (scope, hash) {
      for (var i = 0; i < this.cases.length; i++) {
        ...
        var val = liquidjs.evalExp(branch.val, scope);
        var cond = liquidjs.evalExp(this.cond, scope);
        if (val === cond) {
...
const lexical$3 = liquidjs.lexical;
const groupRE = new RegExp(`^(?:(${lexical$3.value.source})\\s*:\\s*)?(.*)$`);
const candidatesRE = new RegExp(lexical$3.value.source, 'g');
...
var cycle = function (liquid) {
      liquid.registerTag('cycle', {
            ...
            parse: function (tagToken, remainTokens) {
                  ...
                  assert_1(match, `illegal tag: ${tagToken.raw}`);
                  ...
                  assert_1(this.candidates.length, `empty candidates: ${tagToken.raw}`);
            },

            render: function (scope, hash) {
                  var group = liquidjs.evalValue(this.group, scope);
                  ...
                  return anyPromise.resolve(liquidjs.evalValue(candidate, scope));
            }
      });
};

const lexical$4 = liquidjs.lexical;


var decrement = function (liquid) {
  liquid.registerTag('decrement', {
    parse: function (token) {
      var match = token.args.match(lexical$4.identifier);
      assert_1(match, `illegal identifier ${token.args}`);
...
const lexical$5 = liquidjs.lexical;
const mapSeries$2 = promise.mapSeries;

const RenderBreakError$2 = liquidjs.Types.RenderBreakError;

const re$2 = new RegExp(`^(${lexical$5.identifier.source})\\s+in\\s+` + `(${lexical$5.value.source})` + `(?:\\s+${lexical$5.hash.source})*` + `(?:\\s+(reversed))?` + `(?:\\s+${lexical$5.hash.source})*$`);

var _for = function (liquid) {
  liquid.registerTag('for', {
    ...
    parse: function (tagToken, remainTokens) {
      ...
      var p;
      var stream = liquid.parser.parseStream(remainTokens).on('start', () => p = this.templates).on('tag:else', () => p = this.elseTemplates).on('tag:endfor', () => stream.stop()).on('template', tpl => p.push(tpl)).on('end', () => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
    ...
    render: function (scope, hash) {
      var collection = liquidjs.evalExp(this.collection, scope);

      if (!Array.isArray(collection)) {
        if (underscore.isString(collection) && collection.length > 0) {
          collection = [collection];
        } else if (underscore.isObject(collection)) {
      ...
      var html = '';
      return mapSeries$2(contexts, context => {
        return Promise.resolve().then(() => scope.push(context)).then(() => liquid.renderer.renderTemplates(this.templates, scope)).then(partial => html += partial).catch(e => {
          if (e instanceof RenderBreakError$2) {
            html += e.resolvedHTML;
        ...
        if (e instanceof RenderBreakError$2 && e.message === 'break') {
          return;
        }
        throw e;
      }).then(() => html);
...
var _if = function (liquid) {
  liquid.registerTag('if', {
    ...
    parse: function (tagToken, remainTokens) {
      ...
      var p;
      var stream = liquid.parser.parseStream(remainTokens).on('start', () => this.branches.push({
        cond: tagToken.args,
        templates: p = []
      })).on('tag:elsif', token => {
        this.branches.push({
      ...
      }).on('tag:else', () => p = this.elseTemplates).on('tag:endif', token => stream.stop()).on('template', tpl => p.push(tpl)).on('end', x => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
    ...
    render: function (scope, hash) {
      for (var i = 0; i < this.branches.length; i++) {
        ...
        var cond = liquidjs.evalExp(branch.cond, scope);
        if (liquidjs.isTruthy(cond)) {
          return liquid.renderer.renderTemplates(branch.templates, scope);
        }
      }
      return liquid.renderer.renderTemplates(this.elseTemplates, scope);
...
const lexical$6 = liquidjs.lexical;
const withRE = new RegExp(`with\\s+(${lexical$6.value.source})`);
...
var include = function (liquid) {
  liquid.registerTag('include', {
    parse: function (token) {
      ...
      match = lexical$6.value.exec(token.args);
      if (match) {
        this.value = match[0];
      }
    ...
    render: function (scope, hash) {
      var filepath = scope.opts.dynamicPartials ? liquidjs.evalValue(this.value, scope) : this.staticValue;
      assert_1(filepath, `cannot include with empty filename`);
      ...
      return liquid.getTemplate(filepath, scope.opts.root).then(templates => {
        scope.push(hash);
...
const lexical$7 = liquidjs.lexical;

var increment = function (liquid) {
  liquid.registerTag('increment', {
    parse: function (token) {
      var match = token.args.match(lexical$7.identifier);
      assert_1(match, `illegal identifier ${token.args}`);
      this.variable = match[0];
    },
    render: function (scope, hash) {
...
const lexical$8 = liquidjs.lexical;
...
var layout = function (liquid) {
  liquid.registerTag('layout', {
    parse: function (token, remainTokens) {
      ...
      match = lexical$8.value.exec(token.args);
      if (match) {
        this.layout = match[0];
      }
    ...
    render: function (scope, hash) {
      var layout = scope.opts.dynamicPartials ? liquidjs.evalValue(this.layout, scope) : this.staticLayout;
      assert_1(layout, `cannot apply layout with empty filename`);

      // render the remaining tokens immediately
      scope.opts.blockMode = 'store';
      return liquid.renderer.renderTemplates(this.tpls, scope).then(html => {
  ...
  liquid.registerTag('block', {
    parse: function (token, remainTokens) {
      ...
      this.tpls = [];
      var stream = liquid.parser.parseStream(remainTokens).on('tag:endblock', () => stream.stop()).on('template', tpl => this.tpls.push(tpl)).on('end', () => {
        throw new Error(`tag ${token.raw} not closed`);
      });
      stream.start();
    },
    render: function (scope) {
      return anyPromise.resolve(scope.opts.blocks[this.block]).then(html => html === undefined
...
var raw = function (liquid) {
  liquid.registerTag('raw', {
    ...
    render: function (scope, hash) {
      ...
      return anyPromise.resolve(tokens);
    }
  });
};

const mapSeries$3 = promise.mapSeries;
const lexical$9 = liquidjs.lexical;

const re$3 = new RegExp(`^(${lexical$9.identifier.source})\\s+in\\s+` + `(${lexical$9.value.source})` + `(?:\\s+${lexical$9.hash.source})*$`);
...
var tablerow = function (liquid) {
  liquid.registerTag('tablerow', {
    ...
    parse: function (tagToken, remainTokens) {
      ...
      var p;
      var stream = liquid.parser.parseStream(remainTokens).on('start', () => p = this.templates).on('tag:endtablerow', token => stream.stop()).on('template', tpl => p.push(tpl)).on('end', () => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
    ...
    render: function (scope, hash) {
      ...
      return mapSeries$3(contexts, (context, idx) => {
        ...
        scope.push(context);
        return liquid.renderer.renderTemplates(this.templates, scope).then(partial => {
          scope.pop(context);
          html += partial;
          html += '</td>';
          return html;
...
var unless = function (liquid) {
  liquid.registerTag('unless', {
    parse: function (tagToken, remainTokens) {
      ...
      this.elseTemplates = [];
      var p;
      var stream = liquid.parser.parseStream(remainTokens).on('start', x => {
        p = this.templates;
        this.cond = tagToken.args;
      }).on('tag:else', () => p = this.elseTemplates).on('tag:endunless', token => stream.stop()).on('template', tpl => p.push(tpl)).on('end', x => {
        throw new Error(`tag ${tagToken.raw} not closed`);
      });
    ...
    render: function (scope, hash) {
      var cond = liquidjs.evalExp(this.cond, scope);
      return liquidjs.isFalsy(cond) ? liquid.renderer.renderTemplates(this.templates, scope) : liquid.renderer.renderTemplates(this.elseTemplates, scope);
    }
  });
};
...
const isTruthy$1 = syntax.isTruthy;
...
var filters = {
  'abs': v => Math.abs(v),
  'append': (v, arg) => v + arg,
  'capitalize': str => stringify$1(str).charAt(0).toUpperCase() + str.slice(1),
  'ceil': v => Math.ceil(v),
  ...
  'date': (v, arg) => {
    ...
    if (v === 'now') {
      date = new Date();
    } else if (underscore.isString(v)) {
      date = new Date(v);
    }
    return isValidDate(date) ? strftime_1(date, arg) : v;
  },
  'default': (v, arg) => isTruthy$1(v) ? v : arg,
  'divided_by': (v, arg) => v / arg,
  'downcase': v => v.toLowerCase(),
  'escape': escape$1,

  'escape_once': str => escape$1(unescape(str)),
  'first': v => v[0],
  'floor': v => Math.floor(v),
  'join': (v, arg) => v.join(arg),
  'last': v => v[v.length - 1],
  'lstrip': v => stringify$1(v).replace(/^\s+/, ''),
  ...
  'replace': (v, pattern, replacement) => stringify$1(v).split(pattern).join(replacement),
  'replace_first': (v, arg1, arg2) => stringify$1(v).replace(arg1, arg2),
  'reverse': v => v.reverse(),
  'round': (v, arg) => {
    var amp = Math.pow(10, arg || 0);
    return Math.round(v * amp, arg) / amp;
  },
  'rstrip': str => stringify$1(str).replace(/\s+$/, ''),
  'size': v => v.length,
  'slice': (v, begin, length) => v.substr(begin, length === undefined ? 1 : length),
  'sort': (v, arg) => v.sort(arg),
  'split': (v, arg) => stringify$1(v).split(arg),
  'strip': v => stringify$1(v).trim(),
  'strip_html': v => stringify$1(v).replace(/<script.*?<\/script>|<!--.*?-->|<style.*?<\/style>|<.*?>/g, ''),
  'strip_newlines': v => stringify$1(v).replace(/\n/g, ''),
  'times': (v, arg) => v * arg,
  'truncate': (v, l, o) => {
    v = stringify$1(v);
    o = o === undefined ? '...' : o;
  ...
  'upcase': str => stringify$1(str).toUpperCase(),
  'url_encode': encodeURIComponent
};

function escape$1(str) {
  return stringify$1(str).replace(/&|<|>|"|'/g, m => escapeMap[m]);
}

function unescape(str) {
  return stringify$1(str).replace(/&(amp|lt|gt|#34|#39);/g, m => unescapeMap[m]);
}
...
function registerAll(liquid) {
  return underscore.forOwn(filters, (func, name) => liquid.registerFilter(name, func));
}
...
var filters_1 = registerAll;

const statFileAsync$1 = fs_1.statFileAsync;
const readFileAsync$1 = fs_1.readFileAsync;
...
const anySeries$1 = promise.anySeries;


var _engine = {
  init: function (tag$$1, filter, options) {
    if (options.cache) {
      this.cache = {};
    }
    this.options = options;
    this.tag = tag$$1;
    this.filter = filter;
    this.parser = parser(tag$$1, filter);
    this.renderer = render_1();
  ...
  render: function (tpl, ctx, opts) {
    opts = underscore.assign({}, this.options, opts);
    var scope$$1 = scope.factory(ctx, opts);
    return this.renderer.renderTemplates(tpl, scope$$1);
  },
  parseAndRender: function (html, ctx, opts) {
    return anyPromise.resolve().then(() => this.parse(html)).then(tpl => this.render(tpl, ctx, opts));
  },
  renderFile: function (filepath, ctx, opts) {
    opts = underscore.assign({}, opts);
    return this.getTemplate(filepath, opts.root).then(templates => this.render(templates, ctx, opts));
  },
  evalValue: function (str, scope$$1) {
    var tpl = this.parser.parseValue(str.trim());
    return this.renderer.evalValue(tpl, scope$$1);
  },
  registerFilter: function (name, filter) {
    return this.filter.register(name, filter);
  },
  registerTag: function (name, tag$$1) {
    return this.tag.register(name, tag$$1);
  },
  lookup: function (filepath, root) {
    root = this.options.root.concat(root || []);
    root = underscore.uniq(root);
    var paths = root.map(root => path.resolve(root, filepath));
    return anySeries$1(paths, path$$1 => statFileAsync$1(path$$1).then(() => path$$1)).catch(e => {
  ...
  getTemplateFromFile: function (filepath, root) {
    ...
    return this.lookup(filepath, root).then(filepath => {
      if (this.options.cache) {
        var tpl = this.cache[filepath];
        if (tpl) {
          return anyPromise.resolve(tpl);
        }
        return readFileAsync$1(filepath).then(str => this.parse(str)).then(tpl => this.cache[filepath] = tpl);
      } else {
        return readFileAsync$1(filepath).then(str => this.parse(str, filepath));
      }
    });
  },
  ...
  express: function (opts) {
    ...
    return function (filePath, ctx, callback) {
      assert_1(Array.isArray(this.root) || underscore.isString(this.root), 'illegal views root, are you using express.js?');
      opts.root = this.root;
      self.renderFile(filePath, ctx, opts).then(html => callback(null, html)).catch(e => callback(e));
    };
  }
};

function factory$2(options) {
  options = underscore.assign({
    root: ['.'],
    cache: false,
    extname: '',
    dynamicPartials: true,
  ...
  engine.init(tag(), filter$1(options), options);
  return engine;
}

function normalizeStringArray(value) {
  if (Array.isArray(value)) return value;
  if (underscore.isString(value)) return [value];
  return [];
}

factory$2.lexical = lexical;
factory$2.isTruthy = syntax.isTruthy;
factory$2.isFalsy = syntax.isFalsy;
factory$2.evalExp = syntax.evalExp;
factory$2.evalValue = syntax.evalValue;
factory$2.Types = {
  ParseError: error.ParseError,
  TokenizationEroor: error.TokenizationError,
  RenderBreakError: error.RenderBreakError,
  AssertionError: error.AssertionError
};

var liquidjs = factory$2;

// Static website generator. Compiles three things:

const liquid = liquidjs().parseAndRender.bind(liquidjs());
...
const filter$2 = /\.(js|sass|scss|md|hbs|liquid)$/;
...
const compile$1 = (err, file) => {
  ...
  const walked = fsArray_9(process.cwd()).filter(ignore$1).filter(src => filter$2.test(src));
...
nodeWatch(process.cwd(), { recursive: true, filter: filter$2 }, compile$1);
compile$1();
