...
const compile$1 = (err, file) => {
  ...
  walked.filter(isFull).filter(ext('liquid')).forEach(src => {
    const { attributes, body } = frontMatter(fsArray_7(src));
    ...
    liquid(body, { ...attributes, blog }).then(async content => {
      content = await liquid(fsArray_7(templates.liquid[attributes.layout]), { ...attributes, blog, content });
      fsArray_10(output, content);
    });
  });
